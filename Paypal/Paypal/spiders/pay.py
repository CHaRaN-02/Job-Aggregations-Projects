...
from Paypal import items
from scrapy.spidermiddlewares.httperror import HttpError
from twisted.internet.error import DNSLookupError
from twisted.internet.error import TimeoutError, TCPTimedOutError

class PaySpider(scrapy.Spider):
    name = 'pay'
    allowed_domains = ['jobsearch.paypal-corp.com']
    start_urls = ['http://jobsearch.paypal-corp.com/']
    def __init__(self):
        self.priority = 0
        ...
        self.url = 'https://jobsearch.paypal-corp.com/en-US/search?keywords=&facetcountry=&location=&facetcategory=&pagenumber=1'
    ...
    def parse(self, response):
        ...
        total_jobs = sel.css('#job-result-table > tbody > tr *').xpath('@href').extract()
        jobs = []
        for each in total_jobs[4:]:
            job = 'https://jobsearch.paypal-corp.com/' + str(each)
            jobs.append(job)
        ...
        if self.jobs_fetched < self.total_jobs:
            ...
            self.page += 1
            next_page = 'https://jobsearch.paypal-corp.com/en-US/search?keywords=&facetcountry=&location=&facetcategory=&pagenumber={}'.format(self.page)
            yield SplashRequest(
                                url=next_page,
                                callback=self.parse,
                                errback=self.handle_error,
    ...
    def parse_results(self, response):
        # self.counter += 1
        item = items.PaypalItem()
        sele = Selector(response)
        title = sele.xpath('//*[@id="jdp-title-job-title"]/text()').extract()
        if title:
            item['Title'] = title[0].strip()
        block = sele.css('#jdp-job-snapshot-section > div > ul > li *::text').extract()
        employment_type = block[5].strip()
        if employment_type:
            item['EmploymentType'] = employment_type
        location = block[15].strip()
        if location:
            item['Location'] = location
        job_role = block[24].strip()
        if job_role:
            item['EmploymentIndustry'] = job_role
        experience = block[32].strip()
        if experience:
            if experience != 'Not Specified':
                item['Experience'] = experience
        posted = block[40].strip()
        if posted:
        ...
        job_ID = block[48].strip()
        if job_ID:
            item['JobID'] = job_ID
        item['JobURL'] = response.url
        item['Source'] = 'https://jobsearch.paypal-corp.com/'
        item['EmployerName'] = 'PayPal'
        description = sele.css('#jdp-job-description-section > div ::text').extract()
        # cleantext = BeautifulSoup(description[2:-3], "lxml").text
        desc = []
        for i in description[2:-3]:
            i = i.replace('\r\n','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','').replace('\u202f','').strip()
            i = BeautifulSoup(i, "lxml").text
            desc.append(i)
        item['JobDescription'] = ''.join(desc)
        yield item

    def __del__(self):
        state = {
            "url": 'https://jobsearch.paypal-corp.com/en-US/search?keywords=&facetcountry=&location=&facetcategory=&pagenumber=' + str(self.page),
            "page_no": self.page,
